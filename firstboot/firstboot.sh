...
function config() {
    ...
    if apt-get -qq update ; then
        ...
        echo -e "${YEL}Failed to update, this is normal${NC}\nDeleting cdrom from /etc/apt/sources.list and retrying."
        echo -e "Retrying..."
    ...
    if usermod -aG sudo $username ; then
        ...
        echo -e "${RED}Please make sure you executed the command 'su -' and not just 'su' or the script will not run.${NC}"
        exit 1
    fi
...
function interfaces_branch01() {
    ...
    if cat /home/$username/firstboot/interface-conf >> /etc/network/interfaces ; then
        echo -e "${LG}Added succesfully!${NC}\n"
    else
        echo -e "${RED}Failed to add config to /etc/network/interfaces. The config file must be in the same directory as the script.${NC}"
        exit 1
    fi
    echo -e "Restarting the server in 5 second. The IP will change.\nOnce the server is done restarting you can go ahead and run the bootstrap.sh script!"
    for i in {5..1}
    do
        echo -en "${LG}\r$i${NC}"
        sleep 1
...
# Menue to select the branch to configure
selected_item=0
